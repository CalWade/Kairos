...
参考 Mem0 一类系统的“事实抽取 + 更新决策”思路，Kairos 将 LLM 写入过程拆成两步：
...
参考时间化事实建模思路，Kairos 在新旧记忆冲突时不硬删除旧记忆，而是通过 `invalid_at`、`expired_at`、`superseded_by` 保留历史。
...
Kairos 会参考现有 memory 系统的成熟方向，但不把未经验证的外部项目当作候选片段生成算法的正确性证明。

当前边界：

- Mem0 一类系统可参考“事实抽取 + 更新决策”；
- Letta / MemGPT 可参考“长期记忆不应全量塞进上下文，而应工具化检索”；
- Graphiti / Zep 可参考“时间化事实和非破坏式更新”；
- 长期记忆评测研究可参考“召回、更新、时间推理”等评测维度；
- Candidate Segment Pipeline 是 Kairos 针对飞书高噪声协作流的工程设计，必须用 Candidate Recall / Precision / Token Saved 评测证明。
